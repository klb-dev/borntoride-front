...
export function setupThreeJS(canvas: HTMLCanvasElement) {
  ...
  const skateboards: THREE.Group[] = [];
   // skateboard
  const createSkateboard = (x: number, y: number, z: number): THREE.Group => {
  ...
  for (let i = 0; i < 5; i++) {
   ...
   // skater
  const createSkater = (): THREE.Group => {
  ...
  const animate = () => {
    ...
    skateboards.forEach((sk) => {
      ...
      sk.children.slice(3).forEach((wheel) => {
        (wheel as THREE.Mesh).rotation.x += 0.1;
      });
    });
